...
| 🃏 | **Bundled Order Card** | The [GLP Order Card](https://github.com/mxkissnr/glp-order-card) ships inside this integration and registers itself as a Lovelace resource on setup — just add `type: custom:glp-order-card` to a dashboard, no separate HACS install or resource config |
