...
| Firmware (Machine Firmware) | Zeigt, ob eine neuere Firmware-Version für die Espressomaschine selbst verfügbar ist — Vergleich der installierten Version gegen das neueste passende Release im eigenen GitHub-Projekt der Firmware. Unterstützt Installation: Auslösen startet den eigenen OTA-Update-Ablauf der Maschine. Es gibt keine Live-Fortschrittsanzeige während des OTA — `installed_version` zieht nach, sobald die Maschine ihre neue Version beim nächsten Poll meldet. Nur für Gaggiuino-Maschinen; bei GaggiMate nicht verfügbar (dort existiert keine entsprechende Prüfung). Kann das Add-on das neueste Release aktuell nicht ermitteln (Maschine nicht erreichbar, GitHub-Rate-Limit), zeigt die Entity die installierte Version als aktuell an statt „Unbekannt“, und der zuletzt bekannte Firmware-Stand bleibt über kurze Maschinen-Ausfälle erhalten; sobald die Prüfung wieder greift, springt sie von selbst auf „Update verfügbar“. |

Der **Besuchen**-Link auf der GLP-Geräteseite öffnet das Webinterface der Espressomaschine selbst (`http://<Maschinen-Hostname>`), und das Gerät zeigt die installierte Firmware-Version der Maschine als Software-Version — beides wird gesetzt, sobald die Maschine mindestens einmal erreichbar war.
